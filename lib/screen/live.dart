...
import '../providers/apiList_provider.dart'; // <-- DatingZoneProvider ধরে নেওয়া
...
/// ▶ Play overlay colors/sizes (change as you like)
const kPlayBg      = Color(0x99000000); // semi-black circle (60% opacity)
const kPlayIcon    = Colors.white;      // triangle color
const double kPlayBgSize = 30;          // circle diameter
const double kPlaySize   = 20;          // triangle size
...
class _LivePageState extends State<LivePage> {
  ...
  Widget build(BuildContext context) {
    ...
    final firstLoading = p.items.isEmpty && p.error == null;
...
/// ▶ Center play widget with circular background
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Container(
        ...
        child: Icon(Icons.play_arrow_rounded, color: kPlayIcon, size: kPlaySize),
      ),
    );
  }
}
...
class _TopBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(text, style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w800, height: 1.0)),
    );
  }
}
